# FlagGuard Architecture

FlagGuard is a modular, offline-first static analysis tool combining **ASTs (tree-sitter)**, **Formal Verification (Z3)**, **Knowledge Graphs (NetworkX)**, **Vector Databases (ChromaDB)**, and **Multi-Agent LLM Systems (Ollama)** into a single AI-native pipeline.

---

## 🏗️ High-Level System Architecture

```mermaid
flowchart LR
    subgraph Inputs
        A["Flag Config<br/>JSON/YAML"] 
        B["Source Code<br/>Python/JS"]
    end
    
    subgraph Analysis Engine
        C["Config Parser"]
        D["AST Parser<br/>tree-sitter"]
        E["Path Analyzer"]
        F["SAT Solver<br/>Z3"]
        G["Conflict Detector"]
        H["Dead Code Finder"]
    end

    subgraph AI Intelligence Layer
        I["AST Chunker"]
        J["ChromaDB<br/>Vector Store"]
        K["Knowledge Graph<br/>NetworkX"]
        L["Hybrid Retriever"]
        M["Coder Agent<br/>LLM"]
        N["Verifier Agent<br/>Z3"]
    end
    
    subgraph Output Interfaces
        O["LLM Explainer<br/>Gemma 2B"]
        P["Report Generator"]
        Q["CLI Output"]
        R["Web UI<br/>Gradio"]
        S["REST API<br/>FastAPI"]
    end
    
    A --> C --> F
    B --> D --> E --> F
    D --> I --> J
    D --> K
    F --> G --> L
    J --> L
    K --> L
    L --> M --> N
    N -->|"Retry if unsafe"| M
    N -->|"Verified ✅"| O --> P
    F --> H --> O
    P --> Q
    P --> R
    P --> S
```

---

## 🧩 Core Components

### 1. Parsers
- **Config Parser**: Reads feature flag definitions (LaunchDarkly, Unleash, Generic) and extracts flag states, dependencies, and variations.
- **AST Scanner**: Uses `tree-sitter` to scan Python and JavaScript/TypeScript source code to detect patterns like `is_enabled("flag_name")`.

### 2. Analysis Engine
- **FlagSATSolver**: Encodes feature flag dependencies and codebase usage as boolean logic constraints using Microsoft's Z3 SMT solver.
- **Conflict Detector**: Proposes mathematical states to the SAT solver to find impossible conditions (e.g., Flag A depends on Flag B, but Flag B is hardcoded to `false`).
- **Dead Code Finder**: Identifies code paths that require an impossible combination of flags to execute.

### 3. AI Intelligence Layer (GraphRAG)

| Component | Technology | Module | Description |
|-----------|-----------|--------|-------------|
| **AST Chunker** | tree-sitter | `rag/ingester.py` | Function-level semantic code chunking with metadata (replaces naive line-based splitting) |
| **Vector Store** | ChromaDB + SentenceTransformers | `rag/store.py` | Persistent embeddings for semantic similarity search |
| **Knowledge Graph** | NetworkX DiGraph | `ai/graph.py` | Directed function call graph with reverse BFS for transitive impact analysis |
| **Hybrid Retriever** | ChromaDB + NetworkX | `rag/retriever.py` | Dual-strategy retrieval with intelligent merge/dedup/ranking |
| **Coder Agent** | Ollama / Gemma 2B | `ai/agent.py` | LLM-powered code patch generation grounded in RAG context |
| **Verifier Agent** | Z3 SMT Solver | `ai/agent.py` | Formal verification of proposed patches (no new conflicts allowed) |
| **Remediation Loop** | Agentic orchestration | `ai/agent.py` | Coder → Verifier → Retry (max 3) with full reasoning chain tracking |

#### Data Flow: Conflict → Verified Fix

```mermaid
flowchart TD
    A["Z3 Detects Conflict"] --> B["Hybrid Retriever"]
    B --> C["ChromaDB Semantic Search<br/>Top-5 similar code chunks"]
    B --> D["NetworkX Graph Traversal<br/>All transitive callers"]
    C --> E["Merge & Deduplicate<br/>Boost items found by both"]
    D --> E
    E --> F["Coder Agent<br/>LLM generates git diff"]
    F --> G{"Verifier Agent<br/>Z3 SAT Check"}
    G -->|"❌ New conflict"| H["Feed error back<br/>to Coder Agent"]
    H --> F
    G -->|"✅ Safe"| I["Show verified patch<br/>+ reasoning chain to user"]
```

### 4. Interfaces
- **Web UI**: A Gradio-based dashboard with RBAC, Plotly charts, dependency graphs, multi-environment management, and the **🤖 AI Remediation** tab showing verified code fixes.
- **REST API**: A FastAPI backend with 31 endpoints, JWT auth, and SQLite/PostgreSQL support.
- **CLI**: A Click-based terminal tool for CI/CD integration.

---

## 🗄️ Database Schema Mapping

FlagGuard persists projects, environments, and scans to a relational database using SQLAlchemy.

| Model | Description |
|-------|-------------|
| `User` | Authentication state with RBAC (`admin`, `analyst`, `viewer`). |
| `Project` | Represents an application or repository being analyzed. |
| `Environment` | Development, Staging, or Production configs with specific `flag_overrides`. |
| `Scan` | Represents a single analysis run, tied to a Project and Environment. |
| `ScanResult` | The large JSON blob containing the conflict report and LLM explanations. |
| `WebhookConfig` | Project-level webhooks triggered asynchronously on scan completion. |
| `AuditLog` | Immutable record of user actions and security events. |
| `Schedule` | Persistent cron jobs for automated background scanning. |
| `PluginConfig` | User-defined custom parsers injected at runtime dynamically. |

---

## 📂 Directory Structure

```text
flagguard/
|-- api/                  # REST API (FastAPI routes)
|   |-- auth.py           # JWT auth + RBAC
|   |-- server.py         # Main Uvicorn app
|
|-- ai/                   # AI Intelligence Layer (NEW)
|   |-- graph.py          # NetworkX Knowledge Graph builder
|   |-- agent.py          # Agentic Remediation Loop (Coder + Verifier)
|
|-- analysis/             # Formal Verification Engine
|   |-- sat_solver.py     # Z3 constraint encoding
|   |-- conflict_detector.py
|   |-- dead_code.py
|
|-- cli/                  # Command-line interface
|   |-- main.py           # Click commands
|
|-- core/                 # Shared infrastructure
|   |-- db.py             # SQLAlchemy session management
|   |-- models/tables.py  # Database ORM classes
|   |-- roles.py          # RBAC enum system
|
|-- parsers/              # Input ingestion
|   |-- launchdarkly.py   # Config parsers
|   |-- ast/              # Tree-sitter code extractors
|
|-- llm/                  # LLM Integration
|   |-- ollama_client.py  # Ollama API wrapper
|   |-- explainer.py      # RAG-augmented conflict explanations
|   |-- prompts.py        # Prompt templates (including GraphRAG remediation)
|
|-- rag/                  # Retrieval-Augmented Generation
|   |-- ingester.py       # AST-aware function-level code chunker
|   |-- embeddings.py     # SentenceTransformer + Ollama embedding providers
|   |-- store.py          # ChromaDB persistent vector store
|   |-- retriever.py      # Hybrid Retriever (ChromaDB + NetworkX)
|   |-- engine.py         # RAG chat engine orchestrator
|
|-- ui/                   # Web Interface
    |-- app.py            # Gradio Web UI entry point
    |-- tabs/             # Dashboard modules
    |   |-- remediation.py  # 🤖 AI Remediation tab (NEW)
    |-- handlers.py       # Shared business logic
```


---

## 🏗️ High-Level System Architecture

```mermaid
flowchart LR
    subgraph Inputs
        A["Flag Config<br/>JSON/YAML"] 
        B["Source Code<br/>Python/JS"]
    end
    
    subgraph Processing Engine
        C["Config Parser"]
        D["AST Parser<br/>tree-sitter"]
        E["Path Analyzer"]
        F["SAT Solver<br/>Z3"]
        G["Conflict Detector"]
        H["Dead Code Finder"]
    end
    
    subgraph Output Interfaces
        I["LLM Explainer<br/>Gemma 2B"]
        J["Report Generator"]
        K["CLI Output"]
        L["Web UI (Gradio)"]
        M["REST API (FastAPI)"]
    end
    
    A --> C --> F
    B --> D --> E --> F
    F --> G --> I --> J
    F --> H --> I
    J --> K
    J --> L
    J --> M
```

---

## 🧩 Core Components

### 1. Parsers
- **Config Parser**: Reads feature flag definitions (LaunchDarkly, Unleash, Generic) and extracts flag states, dependencies, and variations.
- **AST Scanner**: Uses `tree-sitter` to scan Python and JavaScript/TypeScript source code to detect patterns like `is_enabled("flag_name")`.

### 2. Analysis Engine
- **FlagSATSolver**: Encodes feature flag dependencies and codebase usage as boolean logic constraints using Microsoft's Z3 SMT solver.
- **Conflict Detector**: Proposes mathematical states to the SAT solver to find impossible conditions (e.g., Flag A depends on Flag B, but Flag B is hardcoded to `false`).
- **Dead Code Finder**: Identifies code paths that require an impossible combination of flags to execute.

### 3. Interfaces
- **Web UI**: A complete Gradio-based dashboard featuring Role-Based Access Control, interactive Plotly charts, dependency graphs, and multi-environment management.
- **REST API**: A robust FastAPI backend with 31 endpoints, JWT auth, SQLite/PostgreSQL support, and webhook dispatching.
- **CLI**: A Click-based terminal tool for seamless CI/CD integration.

---

## 🗄️ Database Schema Mapping

FlagGuard persists projects, environments, and scans to a relational database using SQLAlchemy.

| Model | Description |
|-------|-------------|
| `User` | Authentication state with RBAC (`admin`, `analyst`, `viewer`). |
| `Project` | Represents an application or repository being analyzed. |
| `Environment` | Development, Staging, or Production configs with specific `flag_overrides`. |
| `Scan` | Represents a single analysis run, tied to a Project and Environment. |
| `ScanResult` | The large JSON blob containing the conflict report and LLM explanations. |
| `WebhookConfig` | Project-level webhooks triggered asynchronously on scan completion. |
| `AuditLog` | Immutable record of user actions and security events. |
| `Schedule` | Persistent cron jobs for automated background scanning. |
| `PluginConfig` | User-defined custom parsers injected at runtime dynamically. |

---

## 📂 Directory Structure

```text
flagguard/
|-- api/                  # REST API (FastAPI routes)
|   |-- auth.py           # JWT auth + RBAC
|   |-- server.py         # Main Uvicorn app
|
|-- analysis/             # Engine
|   |-- sat_solver.py     # Z3 constraint encoding
|   |-- conflict_detector.py
|   |-- dead_code.py
|
|-- cli/                  # Command-line interface
|   |-- main.py           # Click commands
|
|-- core/                 # Shared infrastructure
|   |-- db.py             # SQLAlchemy session management
|   |-- models/tables.py  # Database ORM classes
|   |-- roles.py          # RBAC enum system
|
|-- parsers/              # Input ingestion
|   |-- launchdarkly.py   # Config parsers
|   |-- ast/              # Tree-sitter code extractors
|
|-- llm/                  # AI Integration
|   |-- ollama_client.py  
|   |-- explainer.py      # Prompt templates
|
|-- ui/                   # Web Interface
    |-- app.py            # Gradio Web UI entry point
    |-- tabs/             # Dashboard modules
    |-- handlers.py       # Shared business logic
```
